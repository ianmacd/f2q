...
/* case 04635704 
   smmu fault: The cmd_done might have come late
   disable cmd_done in case of dma transfer
*/
				...
				struct spi_master *spi)
{
	...
	if (trans_len > fifo_size) {
		...
		spi_disable_cmd_done(mas); /* case 04635704 */
	} else {
					...
					struct spi_transfer *xfer)
{
	...
	if (mas->cur_xfer_mode == FIFO_MODE) /*case 04635704 */
